...
while ($i <= 12) {
    ...
    $squery = $mysqli->query("SELECT type FROM `qurik_logs` WHERE `date` LIKE '%$date' AND `type` = 'SQLi'");
    $scount = mysqli_num_rows($squery);
	
	$array_count['SQLi'][] = $scount;

	// Bad Bots Count
	$bquery = $mysqli->query("SELECT type FROM `qurik_logs` WHERE `date` LIKE '%$date' AND 
	...
	// Proxies Count
	$pquery = $mysqli->query("SELECT type FROM `qurik_logs` WHERE `date` LIKE '%$date' AND `type` = 'Proxy'");
    $pcount = mysqli_num_rows($pquery);
	
	$array_count['Proxies'][] = $pcount;
	
	// Spammers Count
	$spquery = $mysqli->query("SELECT type FROM `qurik_logs` WHERE `date` LIKE '%$date' AND `type` = 'Spammer'");
