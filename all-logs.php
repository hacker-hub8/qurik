...
if (isset($_GET['delete-all'])) {
    $query = $mysqli->query("TRUNCATE TABLE `qurik_logs`");
}
?>   
				...
				<div class="col-md-12">
				    ...
				    <div class="card card-primary card-outline">
						<div class="card-header">
							...
							<a href="?delete-all" class="btn btn-flat btn-sm btn-danger float-sm-right" data-toggle="tooltip" title="Delete all logs"><i class="fas fa-trash"></i> Delete All</a>
						</div>
						<div class="card-body">
...
<table id="dt-basiclogs" class="table table-bordered table-hover table-sm" width="100%">
									...
									<tbody>
<?php
$sql = $mysqli->query("SELECT id, ip, date, time, browser, browser_code, os, os_code, country, country_code, type FROM `qurik_logs` ORDER by id DESC");
while ($row = mysqli_fetch_assoc($sql)) {
    ...
    echo '
										  ...
										  <td>
                                            <a href="log-details.php?id=' . $row['id'] . '" class="btn btn-flat btn-primary btn-sm" data-toggle="tooltip" title="Log Details"><i class="fas fa-tasks"></i></a>
											<a href="?delete-id=' . $row['id'] . '" class="btn btn-flat btn-danger btn-sm" data-toggle="tooltip" title="Delete Log"><i class="fas fa-trash"></i></a>
										  </td>
										</tr>
';
}
